...
func main() {
	...
	r := mux.NewRouter()
	r.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) { fmt.Fprintln(w, "update service") })
	r.HandleFunc("/personas/{documento}", controllers.ActualizarPersona).Methods("PUT")

	port := ":8083"
	fmt.Printf("Update service escuchando en %s\n", port)
